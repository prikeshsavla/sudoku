class SudokuCanvasGame {
  ...
  startNewGame(difficulty = "medium") {
    ...
    const removeCount =
      { easy: 35, medium: 45, hard: 52, expert: 58 }[difficulty] || 45;
  ...
  handleCellInput(num) {
    ...
    if (this.isNoteMode) {
      ...
      if (num === 0 || prevVal === num) {
        ...
        if (
          prevVal !== newValEmpty ||
          [...prevNotes].join(",") !== [...newNotesEmpty].join(",")
        ) {
          this.undoStack.push({
            row,
            col,
        ...
        const isValidPlacement = this.isValidPlacement(
          this.currentBoard,
          row,
          col,
          num,
        ...
        // Place the number and remove that number from notes in peer cells only when the placement is valid.
  ...
  render() {
    ...
    const activeHighlight = this.highlightNumber;
    ...
    if (activeHighlight !== null && activeHighlight !== 0) {
      for (let r = 0; r < 9; r++) {
        for (let c = 0; c < 9; c++) {
          if (this.currentBoard[r][c] === activeHighlight) {
            // Highlight the full row, column and box for each matching value.
  ...
  checkWinCondition() {
    ...
    document.getElementById("victoryTime").textContent =
      `Your time: ${this.formatTime(this.secondsElapsed)}`;
  ...
  setupEventListeners() {
    ...
    this.canvas.addEventListener("touchstart", handleCanvasPointer, {
      passive: false,
    });
    ...
    document
      ...
      .addEventListener("click", () => this.handleCellInput(0));
    document
    ...
    noteBtn.addEventListener("click", () => {
      ...
      this.showToast(
        this.isNoteMode ? "Pencil Note Mode ON" : "Normal Entry Mode ON",
      );
    });
    ...
    document
      .getElementById("victoryNewGameBtn")
  ...
  startTimer() {
    ...
    this.timerInterval = setInterval(() => {
      if (!this.isPaused && !this.isVictory) {
        this.secondsElapsed++;
        document.getElementById("timerDisplay").textContent = this.formatTime(
          this.secondsElapsed,
        );
